...
int CWndCharImage::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	// 돋보기창을 만든다, 그리고 숨겨져 있다
	BOOL bRet = m_wndMagnifier.CreateEx(
		NULL, 
		AfxRegisterWndClass( CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW ),
		_T("CharacterMagnifier"),
		WS_CHILD,
		CRect(0,0,0,0),
		::AfxGetMainWnd(),
		0x1345 );
	...
	// DIB 빗맵 값에 대한 메모리 할당, 그리고 DDB 빗맵에 대한 기본을 생성한다.
