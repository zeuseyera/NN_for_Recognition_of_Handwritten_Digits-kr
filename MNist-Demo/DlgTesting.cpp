...
void CDlgTesting::OnButtonStartTesting() 
{
	...
	dlg.m_strStartingPatternNum.Format( _T("시작이미지 순번 (현재 %d)"), 
	...
	if ( iRet == IDOK )
	{
		...
		if ( bRet != FALSE )
		{
			...
			CString str;
			str.Format( _T("평가 시작됨 \r\n%s 집합 선택됨\r\n\r\n부정확한 인식 이미지 목록:\r\n"),
				( (m_iWhichImageSet==0) ? _T("벼림이미지") : _T("평가이미지") ) );
...
{	
	...
	if ( wParam == 1 )
	{
		...
		str.Format( _T("현재 평가중인 이미지 순번: %d"), pos );
	...
	else if ( wParam == 2 )
	{
		...
		str.Format( _T("%3d : 이미지순번 %3d\t실제값 = %3d\t인식값 = %3d \r\n"), 
